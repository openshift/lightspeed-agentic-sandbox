...
class TestParseMCPServers:
    ...
    def test_missing_ca_file_fails_config(self, tmp_path: Path):
        servers_json = json.dumps(
            [
                {
                    "name": "test",
        ...
        with (
            ...
            pytest.raises(MCPConfigError, match="caFile is missing, unreadable, or invalid"),
        ):
            parse_mcp_servers()

    def test_invalid_ca_file_fails_config(self, tmp_path: Path):
    ...
    def test_relative_ca_file_fails_config(self):
        servers_json = json.dumps(
            [{"name": "test", "url": "https://test:8443/mcp", "caFile": "ca.crt"}]
        )
        with (
            patch.dict(os.environ, {"LIGHTSPEED_MCP_SERVERS": servers_json}),
            pytest.raises(MCPConfigError, match="caFile must be a non-empty absolute path"),
        ):
            parse_mcp_servers()
